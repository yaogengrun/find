...
def yolo_body(inputs, num_anchors, num_classes, phi=0):
    ...
    # 第一个特征层
    x, y1 = make_last_layers(efficientnet.output, int(filters_outs[2]), num_anchors*(num_classes+5))
    x = compose(
            DarknetConv2D_BN_Leaky(int(filters_outs[1]), (1,1)),
            UpSampling2D(2))(x)
    x = Concatenate()([x,feat2])
    # 第二个特征层
    x, y2 = make_last_layers(x, int(filters_outs[1]), num_anchors*(num_classes+5))
    x = compose(
            DarknetConv2D_BN_Leaky(int(filters_outs[0]), (1,1)),
            UpSampling2D(2))(x)
    x = Concatenate()([x,feat1])
    # 第三个特征层
    x, y3 = make_last_layers(x, int(filters_outs[0]), num_anchors*(num_classes+5))
